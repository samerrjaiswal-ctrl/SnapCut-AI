    ...
    try {
      ...
      if (session?.userId) {
        try {
          ...
          toast.message("Result is ready, but saving to history failed.");
        }
      }
    } catch (error) {
      ...
      <div className="flex-1 px-container-margin-mobile md:px-container-margin-desktop pb-8">
        <div className="max-w-7xl mx-auto h-full flex flex-col lg:flex-row gap-gutter">
          <section className="flex-1 bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden flex flex-col min-h-[400px]">
            ...
            <div className="flex-1 p-6 flex items-center justify-center bg-surface-container-low overflow-hidden">
              {!imageUrl ? (
                <UploadArea
                  onFile={onFile}
                ...
                <img
                  src={imageUrl}
                  alt="Original upload"
                  className="max-w-full max-h-[480px] object-contain rounded border border-outline-variant"
          ...
          <section className="flex-1 bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden flex flex-col min-h-[400px]">
            ...
            <div className="flex-1 p-6 flex items-center justify-center bg-surface">
              {status === "processing" ? (
                <ProcessingState message="Reconstructing the background…" className="w-full" />
              ) : status === "error" ? (
                ...
                <p className="font-body-md text-body-md text-on-surface-variant text-center max-w-sm">
                  Upload an image and click Remove Text to see the cleaned result.
                </p>
              )}
            </div>
            <div className="p-4 border-t border-outline-variant bg-surface-bright flex justify-end">
              <button
                ...
                className="flex items-center gap-2 bg-secondary text-on-secondary px-4 py-2 rounded-lg font-label-md text-label-md disabled:opacity-50"
              >
